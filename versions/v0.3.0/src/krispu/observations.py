"""Validated measured coordinates, responses, and LOO eligibility."""
...
class ObservationSet:
    ...
    loo_eligible: NDArray[np.bool_] | ArrayLike | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.loo_eligible is None:
            eligible = np.ones(len(X), dtype=bool)
        else:
            eligible_values = np.asarray(self.loo_eligible)
            if eligible_values.dtype != np.bool_:
                raise ValueError("loo_eligible must be an explicit Boolean mask.")
            eligible = eligible_values.reshape(-1)
        if len(eligible) != len(X):
            raise ValueError("loo_eligible must have one Boolean value per observation.")
        if not np.any(eligible):
            raise ValueError("At least one observation must be eligible for LOO.")
        variances = None
        ...
        object.__setattr__(self, "loo_eligible", eligible)
    ...
    @property
    def loo_eligible_indices(self) -> NDArray[np.int_]:
        return np.flatnonzero(self.loo_eligible)
    ...
    def subset(self, indices: ArrayLike) -> ObservationSet:
        ...
        return ObservationSet(
            self.X[selected], self.y[selected], self.loo_eligible[selected], variances
        )
