...
def panel_figure(
    ...
    *,
    ...
    figure.suptitle(
        ...
        f"n={state.sample_count} | family={state.selected_kernel_id} | "
        f"ARD=[{scales_text}] | NRMSE={state.metrics.nrmse:.4g} | R²={state.metrics.r2:.4g}"
    )
    result = getattr(state, "kernel_selection_result", None)
    badge = None
    if result is not None and result.reselection_event.reselection_triggered:
        if (
            result.previous_kernel_id not in (None, result.selected_kernel_id)
            and result.switch_accepted
        ):
            badge = f"SWITCH: {result.previous_kernel_id} -> {result.selected_kernel_id}"
        else:
            badge = "RESELECTION CHECK"
    elif result is not None and result.previous_kernel_id is None:
        badge = "OPTIMIZED"
    if badge:
        figure.text(
            0.5, 0.985, badge, ha="center", va="top", fontsize=11, weight="bold", color="crimson"
        )
    return figure
...
def _uncertainty(state: Any) -> tuple[np.ndarray, str]:
    ...
    if (
        state.method == "raw_jackknife_sensitivity"
        and state.jackknife_field_sensitivity is not None
    ):
        return state.jackknife_field_sensitivity, "raw buffered-jackknife field sensitivity"
