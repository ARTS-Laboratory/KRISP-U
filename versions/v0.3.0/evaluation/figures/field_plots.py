...
def plot_uncertainty_components(state: Any, path: Path, save_pdf: bool = False) -> None:
    if state.jackknife_field_sensitivity is None:
        return
    x, y, shape = _grid(state.evaluation_points)
    values = [
        state.jackknife_field_sensitivity,
        state.kernel_support_deficit,
        state.krispu_uncertainty,
        state.metrics.absolute_error,
    ]
    labels = [
        "Buffered-jackknife field sensitivity",
...
def plot_sampling_paths(
    ...
    axes = np.asarray(axes).reshape(-1)
    grid_points = np.asarray(final_states[next(iter(final_states))].evaluation_points)
    values = np.asarray(final_states[next(iter(final_states))].true_field)
    ...
    for axis in axes[n:]:
        axis.axis("off")
    figure.suptitle(f"Sampling paths | {getattr(field, 'name', grid_points.shape[0])}")
    _save(figure, path, save_pdf)
...
def plot_uncertainty_error(state: Any, path: Path, save_pdf: bool = False) -> None:
    pairs = [
        ...
        ("jackknife_field_sensitivity", state.jackknife_field_sensitivity),
...
def plot_component_evolution(
    ...
    for key in (
        "mean_posterior_std",
        "mean_jackknife_field_sensitivity",
...
def plot_dominant_jackknife_diagnostics(states: list[Any], path: Path, save_pdf: bool = False) -> None:
    """Show buffered-jackknife spread maps and the driving folds."""
    ...
    for axis, state in zip(axes, selected, strict=True):
        x, y, shape = _grid(state.evaluation_points)
        values = state.jackknife_field_sensitivity.reshape(shape)
        image = axis.pcolormesh(x, y, values, shading="auto", cmap="magma")
        eligible = state.observed_jackknife_eligible
        axis.scatter(
        ...
        dominant = state.dominant_jackknife_observation_index
        title = f"n={state.sample_count}; dominant fold={dominant}"
        axis.set(xlim=(-1, 1), ylim=(-1, 1), aspect="equal", title=title)
        axis.legend(fontsize=7, loc="best")
        figure.colorbar(image, ax=axis, shrink=0.8, label="Buffered-jackknife field sensitivity")
    figure.suptitle(f"Dominant buffered-jackknife fold diagnostics | {selected[0].field}")
    _save(figure, path, save_pdf)
