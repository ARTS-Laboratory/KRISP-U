...
from evaluation.figures.kernel_history import plot_kernel_history
from evaluation.figures.sequential import save_sequential_visuals
from evaluation.metrics.kernel_metrics import aggregate_recovery_rows, kernel_auc_rows
from evaluation.metrics.reconstruction import nrmse_auc
from evaluation.reports.kernel_report import write_kernel_report
from evaluation.runners.config import jackknife_config, load_config, prepare_suite_output
...
def run_kernel_selection_study(
    ...
    for field_index, field_name in enumerate(config["fields"]):
        ...
        for trial in range(int(config["trials"])):
            ...
            initial, initial_jackknife_eligible = initial_design(
            ...
            reference_states = run_sequential_design(
                ...
                gpr_config=GPRConfig(alpha=1e-6, random_state=trial_seed + 90, jackknife=jackknife_config(config)),
            ...
            for mode_index, mode in enumerate(MODE_NAMES):
                ...
                states = run_sequential_design(
                    ...
                    trial=trial,
                    true_evaluation=true_evaluation,
                    gpr_config=GPRConfig(alpha=1e-6, random_state=trial_seed + 100 + mode_index, jackknife=jackknife_config(config)),
                    minimum_normalized_distance=_minimum_normalized_distance(config),
                    boundary_margin=float(config["initial_boundary_margin"]),
            ...
            jackknife_final = isolated["support_adjusted_krispu"][-1]
                ...
                paired_rows.append(
                    {
                        ...
                        "trial": trial,
                        "baseline": method,
                        "delta_nrmse": jackknife_final.metrics.nrmse - baseline_final.metrics.nrmse,
                        "krispu_nrmse": jackknife_final.metrics.nrmse,
    ...
    _write_csv(output / "kernel_recovery_matrix.csv", aggregate_recovery_rows(recovery_rows))
    _write_csv(output / "nrmse_auc.csv", kernel_auc_rows(study_a_rows))
    ...
    plot_kernel_history(history_rows, output / "figures" / "kernel_history_records.png")
    write_kernel_report(output, study_a_final, history_rows, str(config["experiment_name"]))
    return output
...
def _manual_spec(family: str) -> dict[str, Any]:
    family_to_type = {
        "rough_multiscale": "rational_quadratic_ard",
        "periodic": "gaussian_ard",
        "trend_plus_local": "matern_32_ard",
        "rough_single_scale": "exponential_ard",
    }
    return {
        "type": family_to_type.get(family, "matern_52_ard"),
        "length_scale_initial": [0.35, 0.35],
...
def _manual_mismatch_spec(family: str) -> dict[str, Any]:
    if family in {"rough_multiscale", "rough_single_scale"}:
        return {"type": "gaussian_ard", "length_scale_initial": [0.4, 0.4]}
    return {"type": "exponential_ard", "length_scale_initial": [0.12, 0.12]}
...
def _kernel_family(kernel_id: str) -> str:
    if "rational_quadratic" in kernel_id:
        return "multiscale"
    if "52" in kernel_id or "gaussian" in kernel_id:
        return "smooth"
    if "spherical" in kernel_id or "wendland" in kernel_id:
        return "compact"
    return "rough"
...
def _write_figures(
    ...
    axes[0].set_title("Candidate buffered-jackknife scores")
    ...
    for field in fields:
        ...
        if field_candidates:
            ...
            metrics = (
                ...
                ("buffered_predictive_log_score", "Buffered predictive log score"),
                ("upper_tail_normalized_absolute_error", "Upper-tail normalized absolute error"),
                ("buffered_predictive_log_score", "Buffered predictive log score"),
                ("upper_tail_normalized_absolute_error", "Upper-tail normalized absolute error"),
...
def _write_figures_legacy(
    ...
    axes[0, 0].set(title="Buffered-jackknife predictive log score", xlabel="sample count")
    axes[0, 1].plot(
        [row["sample_count"] for row in candidate_rows],
        [row["buffered_predictive_log_score"] for row in candidate_rows],
        ".",
    )
    axes[0, 1].set(title="Buffered predictive log score", xlabel="selection event")
    axes[1, 0].plot(
        [row["sample_count"] for row in candidate_rows],
        [row["upper_tail_normalized_absolute_error"] for row in candidate_rows],
        ".",
    )
    axes[1, 0].set(title="Upper-tail normalized absolute error", xlabel="selection event")
    axes[1, 1].plot(
        [row["sample_count"] for row in candidate_rows],
...
def _write_report(
    ...
    lines = [
        ...
        "",
        "The primary reselection criterion is buffered predictive log score, with upper-tail normalized absolute error as the tie-breaker.",
        "",
        "Degeneracy rules include nonfinite fits, failed factorizations, bound-hitting scales, white-noise dominance, extreme condition numbers, and invalid buffered-jackknife predictions.",
        "",
    ...
    lines.extend(
        (
            ...
            "## Known limitations",
            "",
            "The registry contains only single global anisotropic spatial families; observation noise is a separate diagonal term; and one trial is not evidence of universal kernel recovery.",
            "",
